...
视觉上刻意保持"自己搭的学术主页"的样子：白底、近黑正文、Inter 字体、等宽小字做日期/状态等元数据，只有一种强调色。深色模式为深灰蓝底（`#161b26`），默认跟随访问者系统设置。强调色可在 Appearance 里选（深蓝 / 松绿 / 古铜金 / 酒红 / 墨，都是低饱和的深色调，深色模式下换成对应的柔和浅色），颜色定义在 `style.css` 开头的 `data-accent` 规则和 `appearance.js` 的 `accents` 表中，两处需保持一致。
